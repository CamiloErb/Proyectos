...
const EURUSDrate = () => {
  ...
  useEffect(()=> {
      const setData = () => {
      getEURUSD().then(response => setEurusd(response.conversion_rates? response.conversion_rates.USD : 0) & setUtc(response.time_last_update_utc))
    } 
    setData()
  ...
  return (
    ...
    <UTC>{utc ? utc.substring(5,26) : d.toUTCString()}</UTC>
  </Wrapper>
  )
